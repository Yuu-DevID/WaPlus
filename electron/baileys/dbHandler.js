...
const { normalizeJid, isGroupJid, isNewsletterJid, isLidJid, isUserJid } = require('./parser/jid-utils'); // [JID-UTILS] Baileys-backed JID helpers
    ...
    try {
        const flush = db.transaction(() => {
            for (const [jid, data] of chatSnap) {
                try { statements.updateChatLastMsg.run(data.body, data.msg_type, data.timestamp, data.message_id, data.from_me ?? 0, jid) } catch (_) {}
            }
...
db.exec(`
    CREATE TABLE IF NOT EXISTS messages (
        ...
        -- [STATUS-REPLY] Status reply metadata (v14)
        is_status_reply INTEGER NOT NULL DEFAULT 0,
        quoted_thumbnail_b64 TEXT,
        quoted_status_music TEXT,
    ...
    -- Partial index: undownloaded media rows with jid for fast prefetch queue
    CREATE INDEX IF NOT EXISTS idx_messages_media_pending  ON messages(remote_jid, media_is_downloaded, message_timestamp DESC) WHERE media_is_downloaded = 0;
    CREATE INDEX IF NOT EXISTS idx_messages_starred        ON messages(starred) WHERE starred = 1;
    -- Media gallery per chat (has downloaded or pending)
    CREATE INDEX IF NOT EXISTS idx_messages_participant    ON messages(remote_jid, participant, message_timestamp DESC);
    -- Unread badge counter (fast per-chat)
    CREATE INDEX IF NOT EXISTS idx_messages_unread         ON messages(remote_jid, from_me, status) WHERE from_me = 0 AND is_deleted = 0;
    ...
    -- Status/Story entries (persisted across restarts)
    CREATE TABLE IF NOT EXISTS status_entries (
        ...
        attribution_type TEXT DEFAULT 'none',
        media_url TEXT,
        media_path TEXT,
        media_mimetype TEXT,
        media_size INTEGER,
        ...
        PRIMARY KEY (id, sender_jid)
    );
    CREATE INDEX IF NOT EXISTS idx_status_sender ON status_entries(sender_jid);
    CREATE INDEX IF NOT EXISTS idx_status_expires ON status_entries(expires_at);

    -- ── KV Store — generic key-value persistence ──────────────
    -- Replaces lid_map.json and last_disconnect.json flat files.
    -- Transactional, crash-safe, and always consistent with the rest of the DB.
...
function runMigrations() {
    ...
    const migrations = {
        ...
        10: () => {
            // Add status_entries table for persistence across restarts
            db.exec(`
                CREATE TABLE IF NOT EXISTS status_entries (
                    id TEXT NOT NULL,
                    sender_jid TEXT NOT NULL,
        ...
        // v11: Add from_me column to status_entries for own-status detection
        11: () => {
            const cols = db.pragma('table_info(status_entries)').map(c => c.name);
            if (!cols.includes('from_me')) {
                db.exec('ALTER TABLE status_entries ADD COLUMN from_me INTEGER NOT NULL DEFAULT 0');
                console.log('[AuroraDB] v11: Added from_me to status_entries');
            }
        },

        // v12: Status reply fields — is_status_reply, quoted_thumbnail_b64, quoted_status_music
        12: () => {
            const cols = db.pragma('table_info(messages)').map(c => c.name);
            const add = (col, def) => { if (!cols.includes(col)) db.exec(`ALTER TABLE messages ADD COLUMN ${col} ${def}`); };
            add('is_status_reply',      'INTEGER NOT NULL DEFAULT 0');
            add('quoted_thumbnail_b64', 'TEXT');
            add('quoted_status_music',  'TEXT');
            console.log('[AuroraDB] v12: Added status reply columns');
        },

        // v13: Performance index improvements for fast chat switching.
        // Adds covering indexes on (remote_jid, message_timestamp DESC) — the hot path
        // for getMessages() — plus participant, unread, and media-pending partial indexes.
        // IF NOT EXISTS makes each idempotent; existing DBs get them on first startup.
        13: () => {
            const newIndexes = [
                // Replace single-column media_pending with compound index including remote_jid
                `DROP INDEX IF EXISTS idx_messages_media_pending`,
                `CREATE INDEX IF NOT EXISTS idx_messages_media_pending
                   ON messages(remote_jid, media_is_downloaded, message_timestamp DESC)
                   WHERE media_is_downloaded = 0`,
                // Sender/participant lookup
                `CREATE INDEX IF NOT EXISTS idx_messages_participant
                   ON messages(remote_jid, participant, message_timestamp DESC)`,
                // Unread badge count per chat
                `CREATE INDEX IF NOT EXISTS idx_messages_unread
                   ON messages(remote_jid, from_me, status)
                   WHERE from_me = 0 AND is_deleted = 0`,
                // Chat list covering index
                `CREATE INDEX IF NOT EXISTS idx_chats_list
                   ON chats(archived, pinned DESC, last_message_timestamp DESC)`,
                // Chat group filter
                `CREATE INDEX IF NOT EXISTS idx_chats_group_filter
                   ON chats(is_group, is_community, archived, pinned DESC, last_message_timestamp DESC)`,
            ];
            for (const sql of newIndexes) {
                try { db.exec(sql); } catch (_) {}
            }
            // Refresh query planner stats so SQLite uses the new indexes immediately
            try { db.exec('ANALYZE'); } catch (_) {}
            // Bump page cache + mmap for existing sessions
            try {
                db.pragma('cache_size = -65536');   // 64 MB
                db.pragma('mmap_size = 536870912'); // 512 MB
            } catch (_) {}
            console.log('[AuroraDB] v13: Performance indexes applied');
        },
        14: () => {
            // last_message_from_me: needed by getChatsWithGap/getChatsForDeepGapFill
            // to build correct WAMessageKey for fetchMessageHistory.
            const chatCols = db.prepare('PRAGMA table_info(chats)').all().map(c => c.name)
            if (!chatCols.includes('last_message_from_me')) {
                db.exec('ALTER TABLE chats ADD COLUMN last_message_from_me INTEGER NOT NULL DEFAULT 0')
            }
            try { db.exec(`CREATE INDEX IF NOT EXISTS idx_chats_gap_fill ON chats(last_message_timestamp ASC) WHERE archived = 0`) } catch (_) {}
            try { db.exec(`CREATE INDEX IF NOT EXISTS idx_messages_placeholder ON messages(message_timestamp DESC) WHERE (body IS NULL OR body = '') AND is_deleted = 0`) } catch (_) {}
            console.log('[AuroraDB] v14: Gap-fill indexes + last_message_from_me added')
        },
    };
...
const statements = {
    ...
    insertMessage: db.prepare(`
        INSERT OR IGNORE INTO messages (
            ...
            is_status_reply, quoted_thumbnail_b64, quoted_status_music,
    ...
    getMessagesByJid: db.prepare(`
        SELECT
            ...
            msg.is_status_reply,
            msg.quoted_thumbnail_b64    AS quoted_thumbnail_b64,
            msg.quoted_status_music     AS quoted_status_music,
    ...
    // [FIX-MEDIA-SKIP] Mark media as permanently unavailable (expired CDN / no key).
    // Value -1 = "skipped permanently" — excluded from all pending media queries.
    markMediaExpired:     db.prepare('UPDATE messages SET media_is_downloaded = -1 WHERE id = ?'),
...
const database = {
    ...
    saveMessage(msg, isHistorySync = false, syncType = null) {
        try {
            ...
            const isGroup = isGroupJid(params.remote_jid || '');
    ...
    /**
     * markMediaExpired — set media_is_downloaded = -1 (permanently skipped).
     * Called when prefetch/download encounters no-key or expired CDN (403/AggregateError).
     * Excluded from all pending media queries so we never retry these dead items.
     */
    markMediaExpired(id) {
        if (!id) return;
        try { statements.markMediaExpired.run(id) } catch (_) {}
    },
    ...
    saveChat(chat) {
        ...
        try {
            ...
            const isGroup = isGroupJid(jid) || isNewsletterJid(jid);
            ...
            statements.upsertChat.run({
                ...
                is_community:           isNewsletterJid(jid) || chat.isCommunity ? 1 : 0,
    ...
    upsertChat({ jid, name, isGroup, isCommunity, communityJid, lastMsgAt, unreadDelta }) {
        ...
        const actualIsGroup = isGroup || isGroupJid(cleanJid) || isNewsletterJid(cleanJid);
    ...
    // alias: getPendingMediaForChat (called by client.js priority download queue)
    ...
    // Alias — client.js calls db.getPendingMediaForChat()
    getPendingMediaForChat(jid, limit = 20) {
        return this.getMediaPendingForChat(jid, limit);
    },
    ...
    updateChatLastMsg(jid, { timestamp, message_id, body, msg_type, from_me }) {
        if (!jid) return;
        const cleanJid = normalizeJid(jid)
        _chatBatch.set(cleanJid, { timestamp, message_id, body: body || null, msg_type: msg_type || null, from_me: from_me ? 1 : 0 })
        _scheduleBatchFlush()
    },
    ...
    /**
     * hasExistingData — check real row counts in messages/chats tables.
     *
     * DO NOT use getSyncStatus() for this check — startSync() resets
     * sync_status.total_messages = 0 before the check runs, so it always
     * returns false even when the DB is full of data from a previous session.
     *
     * This is the correct check for "is this a reconnect vs first-time login".
     *
     * Uses EXISTS + LIMIT 1 for maximum speed — stops scanning after first row.
     */
    hasExistingData() {
        try {
            const r = db.prepare(`
                SELECT (
                    ...
                    EXISTS(SELECT 1 FROM chats    LIMIT 1)
                ) AS has_data
            `).get();
            return r?.has_data === 1;
    ...
    /** Save/upsert a single status entry. Called from handleStatusMessage(). */
    upsertStatusEntry(entry) {
        try {
            const HOURS_26 = 26 * 3600;
            const expiresAt = (entry.timestamp || 0) + HOURS_26;
            db.prepare(`
                INSERT OR REPLACE INTO status_entries
                (id, sender_jid, sender_name, from_me, timestamp, source_type, source_type_num,
                 attribution_type, media_url, media_path, media_mimetype, media_size,
                 thumbnail_base64, text, caption, background_color, font, audio_duration,
                 seen, expires_at)
                VALUES
                (@id, @sender_jid, @sender_name, @from_me, @timestamp, @source_type, @source_type_num,
                 @attribution_type, @media_url, @media_path, @media_mimetype, @media_size,
            ...
            `).run({
                ...
                source_type_num:  entry.sourceTypeNum ?? null,
                attribution_type: entry.attributionType || 'none',
                media_url:        entry.mediaUrl   || null,
                media_path:       entry.mediaPath  || null,
                media_mimetype:   entry.mediaMimetype || null,
                media_size:       entry.mediaSize  || null,
                thumbnail_base64: entry.thumbnailBase64 || null,
                text:             entry.text       || null,
                caption:          entry.caption    || null,
                background_color: entry.backgroundColor ?? null,
                font:             entry.font       ?? null,
                audio_duration:   entry.audioDuration ?? null,
                from_me:          entry.fromMe ? 1 : 0,
                seen:             entry.seen ? 1 : 0,
                expires_at:       expiresAt,
            });
        } catch (err) {
            console.error('[AuroraDB] upsertStatusEntry:', err.message);
        }
    },

    /** Mark a single status entry as seen. */
    markStatusEntrySeen(id, senderJid) {
        try {
            db.prepare(`UPDATE status_entries SET seen = 1 WHERE id = ? AND sender_jid = ?`)
              .run(id, senderJid);
        } catch (_) {}
    },

    /** Load all non-expired status entries grouped by sender_jid.
     *  Returns: { [jid]: entry[] } matching the in-memory _statusStore format. */
    loadAllStatusEntries() {
        try {
            const now = Math.floor(Date.now() / 1000);
            const rows = db.prepare(`
                SELECT * FROM status_entries
                WHERE expires_at > ?
                ORDER BY sender_jid, timestamp ASC
            `).all(now);
            ...
            for (const row of rows) {
                ...
                if (!map[jid]) map[jid] = [];
                // Normalize legacy media:// URLs → file:// (Windows-safe)
                let mediaUrl = row.media_url
                if (mediaUrl && mediaUrl.startsWith('media://')) {
                    mediaUrl = 'file:///' + mediaUrl.replace('media://', '').replace(/^\/+/, '')
                }
                // Also verify file still exists on disk
                if (mediaUrl && mediaUrl.startsWith('file:///')) {
                    const fp = row.media_path || decodeURIComponent(mediaUrl.replace('file:///', ''))
                    try { require('fs').accessSync(fp) } catch (_) { mediaUrl = null }
                }
                map[jid].push({
                    id:              row.id,
                    ...
                    sourceTypeNum:   row.source_type_num,
                    attributionType: row.attribution_type || 'none',
                    mediaUrl:        mediaUrl,
            ...
            console.error('[AuroraDB] loadAllStatusEntries:', err.message);
            return {};
        }
    },

    /** Delete expired status entries (call on startup). */
    pruneExpiredStatusEntries() {
        try {
            const now = Math.floor(Date.now() / 1000);
            const result = db.prepare(`DELETE FROM status_entries WHERE expires_at <= ?`).run(now);
            if (result.changes > 0)
                console.log(`[AuroraDB] Pruned ${result.changes} expired status entries`);
        } catch (_) {}
    },
    ...
    // ════════════════════════════════════════════════════════════
    // KV STORE — generic key/value persistence (replaces flat JSON files)
    // ════════════════════════════════════════════════════════════

    /** Get a raw string value from kv_store. Returns null if missing. */
    kvGet(key) {
        try {
    ...
    /** Set a raw string value in kv_store. */
    kvSet(key, value) {
        try {
            db.prepare(`
            ...
            console.warn('[AuroraDB] kvSet error:', err.message);
        }
    },

    /**
     * getLidMap — load the full @lid→real-JID map from DB.
     * Returns a Map<string,string>.
     */
    getLidMap() {
        try {
            const raw = db.prepare('SELECT value FROM kv_store WHERE key = ?').get('lid_map');
    ...
    /**
     * setLidMap — persist the full @lid→real-JID map to DB.
     * Accepts a Map<string,string> or plain object.
     */
    setLidMap(lidMap) {
        try {
            const obj = {};
            if (lidMap instanceof Map) {
                for (const [k, v] of lidMap) obj[k] = v;
            } else {
                Object.assign(obj, lidMap);
            }
            const value = JSON.stringify(obj);
            db.prepare(`
                INSERT INTO kv_store(key, value, updated_at) VALUES('lid_map', ?, unixepoch())
                ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = unixepoch()
            `).run(value);
        } catch (err) {
            console.warn('[AuroraDB] setLidMap error:', err.message);
        }
    },

    /**
     * getLastDisconnectTs — load last clean-disconnect unix-ms timestamp.
     * Returns 0 if not set.
     */
    getLastDisconnectTs() {
        try {
            const raw = db.prepare('SELECT value FROM kv_store WHERE key = ?').get('last_disconnect_ts');
            if (!raw) return 0;
            const ts = Number(raw.value);
            return isFinite(ts) && ts > 0 ? ts : 0;
        } catch (_) { return 0; }
    },

    /**
     * setLastDisconnectTs — persist last clean-disconnect unix-ms timestamp.
     * Pass 0 to clear (after resume sync completes).
     */
    setLastDisconnectTs(ts) {
        try {
            db.prepare(`
                INSERT INTO kv_store(key, value, updated_at) VALUES('last_disconnect_ts', ?, unixepoch())
                ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = unixepoch()
            `).run(String(ts));
        } catch (err) {
            console.warn('[AuroraDB] setLastDisconnectTs error:', err.message);
        }
    },

    /**
     * runMaintenance — call every 30 minutes from client.js via setInterval.
     * Keeps query planner stats fresh and WAL file from growing unbounded.
     */
    runMaintenance() {
        try {
            db.pragma('optimize');
    ...
    /**
     * getChatsWithGap — chats likely to have missed messages after reconnect.
     *
     * For SHORT offline (<1h):  chats with unread OR recent last_message_timestamp.
     * For LONG offline (1h+):   ALL recently-active non-archived chats, because
     *   WA may have cleared unread on another device while we were offline — so
     *   unread_count=0 does NOT mean the chat is fully synced.
     *
     * Returns up to `limit` rows ordered by unread DESC (most urgent first).
     */
    getChatsWithGap(sinceTs, limit = 30) {
        try {
            const sinceSeconds   = Math.floor((sinceTs || 0) / 1000)
            const offlineHours   = sinceTs > 0 ? (Date.now() - sinceTs) / 3_600_000 : 0
            // For long offline, widen the activity window to 2× the offline period
            const cutoffSeconds  = offlineHours > 1
                ? Math.floor(Date.now() / 1000 - offlineHours * 7200)
                : sinceSeconds
            return db.prepare(`
                SELECT jid,
                    last_message_id        AS newest_msg_id,
                    last_message_from_me   AS newest_from_me,
                ...
                LIMIT ?
            `).all(cutoffSeconds, limit)
        } catch (err) {
            console.warn('[AuroraDB] getChatsWithGap error:', err.message)
            return []
        }
    },

    /**
     * getChatsForDeepGapFill — for very long offline (6h+), return ALL
     * recently-active chats regardless of unread state.
     * WA can serve up to 90 days of history via fetchMessageHistory.
     */
    getChatsForDeepGapFill(limit = 50) {
        try {
            return db.prepare(`
                SELECT jid,
                    last_message_id        AS newest_msg_id,
            ...
            console.warn('[AuroraDB] getChatsForDeepGapFill error:', err.message)
            return []
        }
    },

    /**
     * getPlaceholderMessages — stub messages (no body + no type) from last 24h.
     * Used by gap-fill step 3 to requestPlaceholderResend() after reconnect.
     */
    getPlaceholderMessages() {
        ...
        try {
            ...
            console.warn('[AuroraDB] getPlaceholderMessages error:', err.message)
            return []
        }
    },
